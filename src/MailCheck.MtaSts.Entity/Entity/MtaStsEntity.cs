...
namespace MailCheck.MtaSts.Entity.Entity
{
        ...
        IHandle<MtaStsRecordsEvaluated>
    {
        ...
        public async Task Handle(MtaStsScheduledReminder message)
        {
            ...
            MtaStsEntityState state = await LoadState(domainName, nameof(message));
            _log.LogInformation(
                $"Updating MtaStsEntity.MtaStsState from {state.MtaStsState} to {MtaStsState.PollPending} for {domainName}.");
        ...
        public async Task Handle(DomainDeleted message)
        {
            ...
            _log.LogInformation($"Deleted MTA-STS entity with id: {message.Id}.");
        }
        ...
        public async Task Handle(MtaStsRecordsEvaluated message)
        {
            ...
            _log.LogInformation(
                $"Updating MtaStsEntity.MtaStsState from {state.MtaStsState} to {MtaStsState.Evaluated} for {domainName}.");
            ...
            Message createScheduledReminder = new CreateScheduledReminder(
                ...
                "MtaSts",
                domainName,
                _clock.GetDateTimeUtc().AddSeconds(_mtaStsEntityConfig.NextScheduledInSeconds *
                                                   (1 - new Random().NextDouble() * 0.25))
            );
